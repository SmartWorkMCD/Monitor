...
export class MqttService {
		...
		return new Promise((resolve, reject) => {
			try {
				...
				this.client.on("reconnect", () => {
					console.log(
						`MQTT client reconnecting... (attempt ${this.reconnectAttempts + 1})`,
					);
	...
	private handleReconnect(): void {
		...
		if (this.reconnectAttempts < this.maxReconnectAttempts) {
			...
			this.addWarning({
				severity: "high",
				message:
					"Unable to reconnect to Workstation Brain after multiple attempts",
				timestamp: Date.now(),
			});
		}
	}
	...
	private classifyTopic(topic: string): string {
		...
		if (topic.includes("neighbor") || topic.includes("station"))
			return "NETWORK";
	...
	private handleHandTrackingMessage(message: any): void {
		try {
			const baseMsg = message.hand_position ?? message;
			const left = baseMsg.left_hand || baseMsg.left;
			const right =
				baseMsg.right_hand ||
				baseMsg.right ||
				(baseMsg.handR_Wrist_x !== undefined
					? { x: baseMsg.handR_Wrist_x, y: baseMsg.handR_Wrist_y }
					: undefined);
			...
			if (left || right) {
				const handPosition: HandPosition = {
					left_hand: left,
					right_hand: right,
					timestamp: baseMsg.timestamp || message.timestamp || Date.now(),
					grid_cell: baseMsg.grid_cell ?? message.grid_cell,
					in_confirmation_area:
						baseMsg.in_confirmation_area ??
						message.in_confirmation_area ??
						false,
				};
				...
				const cell = baseMsg.grid_cell ?? message.grid_cell;
				if (cell) {
					this.updateGridActivityFromHand(cell);
				}
			}
		} catch (error) {
	...
	private updateGridActivityFromHand(gridCell: {
		row: number;
		col: number;
	}): void {
	...
	private handleTaskCompletionMessage(message: any): void {
		try {
			...
			if (this.currentTaskProgress.completed_tasks.length > 50) {
				this.currentTaskProgress.completed_tasks =
					this.currentTaskProgress.completed_tasks.slice(0, 50);
			}
	...
	private updateTaskFromAssignment(
		taskId: string,
		subtaskId: string,
		products: any,
	): void {
		...
		try {
			...
			if (message.neighbors) {
				...
				const existingConnectionIndex =
					this.currentNeighborsData.ble_connections.findIndex(
				...
				if (existingConnectionIndex >= 0) {
					this.currentNeighborsData.ble_connections[existingConnectionIndex] =
						connectionData;
				} else {
		...
		try {
			if (message.positions || message.connections) {
				this.currentNeighborsData = {
					...
					topology: {
						positions:
							message.positions || this.currentNeighborsData.topology.positions,
						connections:
							message.connections ||
							this.currentNeighborsData.topology.connections,
	...
	private handleProjectorMessage(message: any): void {
		if (message.task && message.subtask && message.progress !== undefined) {
			console.log(
				`Projector task update: ${message.task} - ${message.subtask} (${message.progress}%)`,
			);
	...
	private handleTelemetryMessage(message: any): void {
		if (message.subtask_id && message.duration) {
			console.log(
				`Task completion timing: ${message.subtask_id} - ${message.duration}s`,
			);
	...
	private updateSystemStatus(event: SystemStatusEvent): void {
		...
		if (event.status === "waiting_confirmation" || event.status === "error") {
			const severity: WarningSeverity =
				event.status === "error" ? "high" : "medium";
			this.addWarning({
				severity,
	...
	private updateTask(event: TaskUpdateEvent): void {
		...
		if (existingIndex >= 0) {
			this.currentTaskProgress.current_tasks[existingIndex] =
	...
	private handleRuleEvaluation(event: RuleEvaluationEvent): void {
		if (!event.satisfied) {
			const severity: WarningSeverity = this.mapSeverity(
				event.details || event.rule_id,
			);
			this.addWarning({
				severity,
	...
	private mapSeverity(details: string): WarningSeverity {
		...
		if (
			...
			lowerDetails.includes("anomaly") ||
			lowerDetails.includes("confirmation")
		) {
			return "medium";
		}
		return "low";
	...
	simulateSensorUpdates(): void {
		...
		const simulationInterval = setInterval(() => {
			...
			this.currentSensorData.temperature +=
				Math.floor((Math.random() - 0.5) * 2 * 4) / 4;
			this.currentSensorData.humidity +=
				Math.floor((Math.random() - 0.5) * 3 * 4) / 4;

			this.currentSensorData.temperature = Math.max(
				20,
				Math.min(35, this.currentSensorData.temperature),
			);
			this.currentSensorData.humidity = Math.max(
				30,
